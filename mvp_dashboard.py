...
# (V2 with persistent results + Ruthless AI commentary + User Accounts + Per-User Saved Strategies + Exports)
...
# ---------------------------------------------------------------------
def build_ruthless_ai_commentary(metrics: dict, trades_df: pd.DataFrame) -> str:
    ...
    # formatted with extra spacing for clean bullets
...
def run_mvp_dashboard():
    ...
    with col2:
        ...
        if run_clicked:
            # When button pressed -> run backtest ONCE and save results
            try:
            ...
            try:
                ...
                fig_zoom.add_trace(
                    go.Scatter(
                        ...
                        marker_size=11,
                        marker_color="green" if trow["pnl"] > 0 else "red",
                        name="Trade Entry",
                    )
                )
                ...
                fig_zoom.add_trace(
                    go.Scatter(
                        ...
                        mode="lines",
                        line=dict(
                            color="green" if trow["pnl"] > 0 else "red",
                            width=1.8,
                        ),
                        name="Trade Path",
                    )
    ...
    if trades_df.empty:
        ...
        # Strategy YAML (current text from editor/session)
